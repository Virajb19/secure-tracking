/**
 * Form 6C - Students (Pre-Primary to Class 10)
 * 
 * Form to enter student strength details for Pre-Primary to Class 10.
 */
...
export default function Form6CLowerScreen() {
    ...
    // Fetch existing student strength data
    const { data: existingData, isLoading, refetch } = useQuery({
        queryKey: ['form-6c-lower'],
        queryFn: async () => {
    ...
    useEffect(() => {
        if (existingData?.strengths) {
            ...
            existingData.strengths.forEach((s: any) => {
                const index = updatedData.findIndex(c => c.class_level === s.class_level);
                if (index !== -1) {
                    updatedData[index] = {
                        ...updatedData[index],
            ...
            setFormStatus(existingData.form_status as FormStatus || 'NOT_SUBMITTED');
            setRejectionReason(existingData.rejection_reason || null);
        }
    }, [existingData]);
    ...
    const submitMutation = useMutation({
        mutationFn: async () => {
            ...
            const response = await apiClient.post('/form-6/submit-6c-lower', { strengths: data });
            return response.data;
        },
        onSuccess: () => {
            setFormStatus('SUBMITTED');
            Alert.alert('Success', 'Form 6C submitted successfully!');
        },
        onError: (error: any) => {
            ...
            >
                ...
                >
                    {classData.map((classItem, index) => {
                        const isEditable = formStatus === 'NOT_SUBMITTED' || formStatus === 'DRAFT' || formStatus === 'REJECTED';
                        return (
                            <View key={classItem.class_level} style={styles.classSection}>
                        ...
                        <>
                            ...
                            >
                                {submitMutation.isPending ? (
                                    ...
                                    <Text style={styles.submitButtonText}>Submit Form 6C (Upto Class 10)</Text>
                                )}
                            </TouchableOpacity>
                        </>
                    )}
